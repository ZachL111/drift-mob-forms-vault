...
`drift-mob-forms-vault` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for forms workflows, centered on constraint solving, bounded scenario files, and conflict explanations.
...
The score is deterministic. The current threshold is 171. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 2.
